...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for ghost in livingGhostPositionDistributions:
            max_pos, max_prob = None, 0
            for position, probability in ghost.items():
                if probability >= max_prob:
                    max_pos, max_prob = position, probability
            likely_pos.append(max_pos)
        sorted_pos = sorted(likely_pos, key=lambda (x, y): self.distancer.getDistance(pacmanPosition, (x, y)))
        closest_pos = sorted_pos[0]
        sorted_actions = sorted(legal, key=lambda a: self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, a), closest_pos))
