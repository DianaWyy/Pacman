...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # self.beliefs = P(noisyDistance | TrueDistance) * P(TrueDistance), then normalize
        #              = emissionModel * self.beliefs * normalize
        allPossible = util.Counter()
        if noisyDistance is None:
            jailPos = self.getJailPosition()
            allPossible[jailPos] = 1.0  # captured
        else:
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for old_pos in self.legalPositions:
            new_pdist = self.getPositionDistribution(self.setGhostPosition(gameState, old_pos))
            for new_pos, prob in new_pdist.items():
                new_prob = self.beliefs[old_pos] * prob
                updated_beliefs[new_pos] += new_prob
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for p in self.particles:
                all_p[p] += emissionModel[util.manhattanDistance(p, pacmanPosition)]
        if all_p.totalCount() == 0:
            self.initializeUniformly(gameState)
        else:
            all_p.normalize()
            self.particles = [util.sample(all_p) for i in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        dist = [self.getPositionDistribution(self.setGhostPosition(gameState, p)) for p in self.particles]
        self.particles = [util.sample(d) for d in dist]
    ...
    def initializeParticles(self):
        ...
        while len(self.particles) < self.numParticles:
            if count >= len(positions):
                count = 0
            self.particles.append(positions[count])
            count += 1
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for i in range(len(self.particles)):
            count = 0
            temp = 1
            for d in noisyDistances:
                if d is not None:
                    true_dist = util.manhattanDistance(self.particles[i][count], pacmanPosition)
                    temp *= emissionModels[count][true_dist]
                else:
                    self.particles[i] = self.getParticleWithGhostInJail(self.particles[i], count)
                count += 1
            all_p[self.particles[i]] += temp
        for val in all_p.values():
            if val == 0:
                continue
            else:
                all_p.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            newParticle = [util.sample(getPositionDistributionForGhost(
                setGhostPositions(gameState, newParticle), ghost_i, self.ghostAgents[ghost_i]))
